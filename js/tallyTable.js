 ...
 */
function TallyTable(replicateSetTable){

    var self = this;
    self.replicateSetTable = replicateSetTable;
    self.init();
};
 ...
 */
TallyTable.prototype.init = function(){
    var self = this;
 ...
 * @param
 */
 // TODO: COLLAPSE TALLY TABLE AND REPLICATE SET TABLE INTO SINGLE SCRIPT
TallyTable.prototype.update = function(){
  ...
  var csv_path = "data/genotype_tally_summary_20210126.csv" // create from passed genotype, which is passed from tallyTable
    ...
    d3.csv(csv_path, function(error, data) {
      ...
      d3.select('#tally-table').call(table_plot);
      ...
      table_plot.on('highlight', function(data, on_off){
        if(on_off){//if the data is highlighted
          d3.select(highlighted_div).text(
            'Oops, I just stepped over gene ' + data.GENOTYPE
          );
        }
      });
      ...
      table_plot.on('select', function(data, on_off){
        if(on_off){//if the data is highlighted
          d3.select(selected_div).text(
            'And it was the chosen one ' + data.GENOTYPE
          );
        }
      });
    }); // end d3
...
function _table(targetDiv) {
  ...
  var tableSelect = targetDiv.append("table")
    ...
    // I will leave it for now for simplicity. TODO: remove hard-coded id. //chase's note: funny, i had exactly this note and said the same thing
    .attr("id", "change_to_var_input")
  ...
  $(document).ready(function() {
    table = $('#change_to_var_input').DataTable({
